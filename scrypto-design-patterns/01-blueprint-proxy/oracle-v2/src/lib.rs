...
mod oracle {
    ...
    impl Oracle {
        pub fn instantiate_owned() -> Owned<Oracle> {
            info!("[Oracle v2] instantiate_owned()");
            Self {
        ...
        pub fn instantiate_and_globalize(
            ...
            info!("[Oracle v2] instantiate_and_globalize()");
        ...
        pub fn get_oracle_info(&self) -> String {
            info!("[Oracle v2] get_oracle_info()");
            let info = self.info.clone();
            info!("[Oracle v2] get_oracle_info() info = {:?}", info);
            info
        }

        pub fn set_price(&mut self, base: ResourceAddress, quote: ResourceAddress, price: Decimal) {
            info!(
                "[Oracle v2] set_price() base = {:?} quote = {:?} price = {:?}",
                base, quote, price
            );
        ...
        pub fn get_price(&self, base: ResourceAddress, quote: ResourceAddress) -> Option<Decimal> {
            info!(
                "[Oracle v2] get_price() base = {:?} quote = {:?}",
                base, quote
            );
            let price = self.prices.get(&(base, quote)).map(|price| *price);
            info!("[Oracle v2] get_price() price = {:?}", price);
            price
        }
    }
}
