...
mod yield_amm {
    ...
    extern_blueprint! {
        ...
        YieldTokenizer {
            ...
            fn pt_resource(&self) -> FungibleResourceManager;
            fn yt_resource(&self) -> NonFungibleResourceManager;
            fn underlying_resource(&self) -> FungibleResourceManager;
    ...
    impl YieldAMM {
        ...
        pub fn instantiate_yield_amm(
            ...
            let pt_resource = TOKENIZER.pt_resource();
        ...
        pub fn set_initial_ln_implied_rate(&mut self, initial_rate_anchor: PreciseDecimal) {
            assert_eq!(
            ...
            let rate_scalar = calc_rate_scalar(self.scalar_root, time_to_expiry);

            self.last_ln_implied_rate =
                self.get_ln_implied_rate(time_to_expiry, rate_scalar, initial_rate_anchor);
        ...
        pub fn get_vault_reserves(&self) -> IndexMap<FungibleResourceManager, Decimal> {
        ...
        /// # Returns
        ///
        /// * [`FungibleBucket`] - A fungible bucket of `pool_unit`.
        /// * [`Option<FungibleBucket>`] - An optional fungible bucket of any remainder token.
        ...
        /// * [`FungibleBucket`] - A fungible bucket of LSU tokens.
        ...
        pub fn swap_exact_pt_for_lsu(&mut self, principal_token: FungibleBucket) -> FungibleBucket {
            assert_ne!(
                self.check_maturity(),
                true,
        ...
        pub fn swap_exact_lsu_for_pt(
            ...
            assert_eq!(
                ...
                TOKENIZER.underlying_resource().address()
            );
            ...
            // Calcs the swap
            let required_lsu =
                self.calc_trade(desired_pt_amount, time_to_expiry, market_compute.clone());
            ...
            info!(
                "[swap_exact_lsu_for_pt] Owed PT: {:?}",
                owed_pt_bucket.amount()
            );
        ...
        pub fn swap_exact_lsu_for_yt(
            ...
            let (principal_token, yield_token) = TOKENIZER.tokenize_yield(lsu_token);
        ...
        pub fn swap_exact_yt_for_lsu(
            ...
            let (pt_flash_loan, flash_loan_receipt) =
                self.flash_loan(TOKENIZER.pt_resource(), pt_flash_loan_amount);
            ...
            let optional_return_bucket =
                self.flash_loan_repay(pt_flash_loan_repay, flash_loan_receipt);
        ...
        fn compute_market(&self, time_to_expiry: i64) -> MarketCompute {
        ...
        fn calc_trade(
            ...
            let pre_fee_amount = net_pt_amount
            ...
            let net_asset_fee_to_reserve = fee.checked_mul(self.reserve_fee_percent).unwrap();

            // Trading fee allocated to the reserve based on the direction
            // of the trade.
            let trading_fee = fee.checked_sub(net_asset_fee_to_reserve).unwrap();
            ...
            let net_amount = if net_amount < PreciseDecimal::ZERO {
                ...
                // PT ---> LSU
                net_amount.checked_sub(net_asset_fee_to_reserve).unwrap()
            };
        ...
        fn flash_loan(
            &mut self,
            resource: FungibleResourceManager,
            amount: Decimal,
        ) -> (FungibleBucket, NonFungibleBucket) {
            let flash_loan_receipt = self
                .flash_loan_rm
                .mint_ruid_non_fungible(FlashLoanReceipt {
        ...
        fn flash_loan_repay(
            ...
            let mut flash_loan_receipt_data: FlashLoanReceipt =
                flash_loan_receipt.as_non_fungible().non_fungible().data();
            ...
            assert_eq!(
                self.flash_loan_rm.address(),
                flash_loan_receipt.resource_address()
            );
            assert_eq!(
                flash_loan.resource_address(),
                flash_loan_receipt_data.resource
            );
            assert_eq!(flash_loan_receipt_data.amount, Decimal::ZERO);

            self.pool_component
                .protected_deposit(flash_loan_repay.into());
        ...
        fn get_ln_implied_rate(
            ...
            let exchange_rate = calc_exchange_rate(proportion, rate_anchor, rate_scalar);

            // exchangeRate >= 1 so its ln >= 0
            let ln_exchange_rate = exchange_rate.ln().unwrap();

            let ln_implied_rate = ln_exchange_rate
