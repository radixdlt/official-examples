...
| `flash_loan_rm` | `NonFungibleResourceManager` | The `flash_loan_rm` is a field that contains the `ResourceManager` for the flash loan receipt. It is a receipt minted when taking out flash loans.
...
| Name            | Type            | Arguments       | Type | Returns | Description
| --------------- | --------------- | ----------------- | --------------- | --------------- | --------------- |
| `flash_loan` | Method | `resource`<br>`amount` | `FungibleResourceManager`<br>`Decimal` | A `FungibleBucket` of the flash loan.<br>A `NonFungibleBucket` of the transient flash loan receipt. | A method that allows one to borrow from the pool without collateral so long as the loan is repaid within the same transaction.

```rust
pub fn flash_loan(
    &mut self,
    resource: RFungibleResourceManager,
    amount: Decimal
